...
class PATH {
public:
	int node[MAXN], block[MAXN], nodecnt, blockcnt;
	int len, dev;
...
int dis[MAXN], pre[MAXN], G[MAXN][MAXN], vis[MAXN];
...
int dijkstra(int n) {
	for(int p = 0;p < n;p++) {
		int minV = -1;
		for(int i = 0; i < n; i++) if(!vis[i] && (minV == -1 || dis[i] < dis[minV])) minV = i;
		}
		if (minV == -1) break;
		vis[minV] = true;
		...
		for(int to = 0;to < n;to++)
			if(!vis[to] && !block[minV][to]) {
				...
				if(dis[to] > dis[minV]+len || (dis[to] == dis[minV]+len && minV < pre[to])) {
					dis[to] = dis[minV]+len; pre[to] = minV;
				}
			}
	}
	return 0;
...
int yenLoopless(int s,int t,int n,int k) {
	...
	while (cnt < k && !candidate.empty()) {
		...
		while (dev < p.nodecnt) {
			...
			if (dev == p.dev)
				for (int i = 0; i < p.blockcnt; i++) block[last][p.block[i]] = true;
